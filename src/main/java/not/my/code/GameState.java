...
public class GameState {
	...
	public GameState(int left, int right){
		...
		rScore = 0;
		pause = 90;
		lTeam = new Player[]{new Player(-100,100,false, left), new Player(-200,-200, false, left), new Player(-400, 0, false, left)};
		rTeam = new Player[]{new Player(100,-100,true, right), new Player(200,200, true, right), new Player(400, 0, true, right)};
		ball = new Ball();
		ballStartx = ball.x;
		ballStarty = ball.y;
	}
	...
	public void update(){
		...
		actions[0] = lTeam[0].takeAction(ball.x-0.0001, ball.y-0.0001, ball.vx, ball.vy, lTeam[1].x, lTeam[1].y, lTeam[2].x, lTeam[2].y, rTeam[0].x, rTeam[0].y, rTeam[1].x, rTeam[1].y, rTeam[2].x, rTeam[2].y);
		actions[1] = lTeam[1].takeAction(ball.x-0.0001, ball.y-0.0001, ball.vx, ball.vy, lTeam[0].x, lTeam[0].y, lTeam[2].x, lTeam[2].y, rTeam[0].x, rTeam[0].y, rTeam[1].x, rTeam[1].y, rTeam[2].x, rTeam[2].y);
		actions[2] = lTeam[2].takeAction(ball.x-0.0001, ball.y-0.0001, ball.vx, ball.vy, lTeam[0].x, lTeam[0].y, lTeam[1].x, lTeam[1].y, rTeam[0].x, rTeam[0].y, rTeam[1].x, rTeam[1].y, rTeam[2].x, rTeam[2].y);
		actions[3] = rTeam[0].takeAction(ball.x+0.0001, ball.y+0.0001, ball.vx, ball.vy, rTeam[1].x, rTeam[1].y, rTeam[2].x, rTeam[2].y, lTeam[0].x, lTeam[0].y, lTeam[1].x, lTeam[1].y, lTeam[2].x, lTeam[2].y);
		actions[4] = rTeam[1].takeAction(ball.x+0.0001, ball.y+0.0001, ball.vx, ball.vy, rTeam[0].x, rTeam[0].y, rTeam[2].x, rTeam[2].y, lTeam[0].x, lTeam[0].y, lTeam[1].x, lTeam[1].y, lTeam[2].x, lTeam[2].y);
		actions[5] = rTeam[2].takeAction(ball.x+0.0001, ball.y+0.0001, ball.vx, ball.vy, rTeam[0].x, rTeam[0].y, rTeam[1].x, rTeam[1].y, lTeam[0].x, lTeam[0].y, lTeam[1].x, lTeam[1].y, lTeam[2].x, lTeam[2].y);
		...
		checkPlayerCollisions();if (updateBall()){
			if (replay){
				pause = 90;
				lTeam = new Player[]{new Player(-100,100,false, left), new Player(-200,-200, false, left), new Player(-400, 0, false, left)};
				rTeam = new Player[]{new Player(100,-100,true, right), new Player(200,200, true, right), new Player(400, 0, true, right)};
	...
	public void draw(Graphics g, int width, int height){
		...
		if (pause != 0){
			...
			if (pause > 0){
				g.drawString("" + (int)(pause / 30+1) , -width/2 + 100, -height/2 + 70);
			}
		}
		g.setColor(Color.white);
		g.drawString("" + lScore, -width/2+50, height/2-20);
		g.drawString("" + rScore, width/2-100, height/2-20);
		if (replay){
			g.drawString("" + replayTime/60, 0, -height/2+50);
			g.setColor(Color.yellow);
			g.drawString("REPLAY", width/2-300, -height/2+50);
		}
		else{
			g.drawString("" + time/60, 0, -height/2+50);
		}
	}
}
