...
public class AISoccer extends Application {
    ...
    private static final String VIEW_FILE_1 = "manic0";
    private static final String VIEW_FILE_2 = "manic1";

    private static boolean vs_Andrew = true;
    public static final double ANDREW_MAX_SPEED = 4;
    private static final Class<GeneticNeuralNet> AI_TYPE = GeneticNeuralNet.class;
    private static final String SAVE_PREFIX = "manic";

    private static final int CYCLE = 20;

    private static final int POPULATION_SIZE = 100;
    private static final int ELITES = 15;
    private static final double MUTATION_RATE = 30.0 / NeuralNet.NUM_WEIGHTS;

    private static final int MAX_GEN = 100_000;
    private static final int FRAMES_PER_BACKUP = 40;
    private static final int FRAMES_PER_MATCH = 50_000;
	...
	if (VIEW) {
		...
		if (vs_Andrew)
			...
			// population.get(i).cacheScore(match.getLeftScore());
			...
			population.get(i).cacheScore(match.getLeftScore());
			population.get(i + 1).cacheScore(match.getRightScore());
		    }
		...
		if (vs_Andrew) {
		    ...
		    System.out.format("Min: %.2f, Max: %.2f, Avg: %.2f%n", minScore, maxScore, avgScore);
		    ...
		    double minScore = Collections.max(population).getScoreCache();
		    double maxScore = Collections.min(population).getScoreCache();
		    System.out.format("Min: %.2f, Max: %.2f%n", minScore, maxScore);
		}
