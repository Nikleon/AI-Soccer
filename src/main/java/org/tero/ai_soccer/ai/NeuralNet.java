...
public class NeuralNet implements Comparable<NeuralNet>, Cloneable {
	public static final int[] DEFAULT_SHAPE = { 16, 32, 16, 16, 3 };
	...
	protected SimpleMatrix forwardPropagate(SimpleMatrix inputColumn) {
		for (int layer = 0; layer < depth; layer++) {
			...
			SimpleMatrix preActivations = weights[layer].mult(prevLayer); /* TODO +bias */
			normalize(preActivations);
			activationCache[layer] = (activationFunctions[layer] == null ? ActivationFunctions.DEFAULT
					: activationFunctions[layer]).apply(preActivations);
		}
		activationCacheValid = true;
		return activationCache[depth - 1];
	}

	private void normalize(SimpleMatrix mat) {
		double[] data = mat.getMatrix().data;
	...
	public void decay(double dw) {
		for (WeightMatrix w : weights) {
			double[] data = w.getMatrix().data;
