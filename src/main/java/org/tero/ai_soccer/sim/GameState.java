...
public class GameState {
	...
	private void fillTeams(NeuralNet leftAi, NeuralNet rightAi) {
		// Vector2D[] initialPos = { Vector2D.of(-100, 100), Vector2D.of(-200, -200),
		// Vector2D.of(-400, 0) };
		Vector2D[] initialPos = {
				Vector2D.of(0.8 * Math.random() * (-FIELD_LENGTH), 2 * 0.8 * (Math.random() - 0.5) * (FIELD_WIDTH)),
				Vector2D.of(0.8 * Math.random() * (-FIELD_LENGTH), 2 * 0.8 * (Math.random() - 0.5) * (FIELD_WIDTH)),
				Vector2D.of(0.8 * Math.random() * (-FIELD_LENGTH), 2 * 0.8 * (Math.random() - 0.5) * (FIELD_WIDTH)) };
	...
	public void draw(GraphicsContext g, int width, int height) {
		...
		g.setFill(Color.WHITE);
		g.fillText(String.format("%.2f", lScore), -width / 2 + 50, height / 2 - 20);
		g.fillText(String.format("%.2f", rScore), width / 2 - 200, height / 2 - 20);
		if (isReplaying) {
